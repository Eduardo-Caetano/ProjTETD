    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/50 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        ...
        <label className="mb-4 block">
          <span className="mb-1 block text-sm font-medium text-slate-700">{labelName}</span>
          <select
            value={form.name}
            onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))}
            className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-ocean focus:ring-2 focus:ring-ocean/20"
            required
          >
            {options.map((option) => (
        ...
        <label className="mb-5 block">
          ...
          <textarea
            ...
            rows="3"
            className="w-full resize-none rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-ocean focus:ring-2 focus:ring-ocean/20"
          />
        </label>
